...
/// データベースの内容を、指定されたパスの `SQLite` データベースファイルへ書き込む。同じディレクトリの一時ファイルへ書き込みを完了した場合にのみ、指定されたパスへ改名して既存のファイルを置き換える。事後条件として、種別を持たない折れ線には、その折れ線を構成要素に含む最初のグループの種別を格納する。グループの構成要素のうち折れ線の列に存在しない識別子は無視し、重複する識別子は 1 つに纏める。事前条件として、各折れ線は 1 点以上の座標を持ち、種別を指定されているか種別を持つグループに属していること。
///
/// # Errors
///
...
pub fn write_database(path: &Path, content: &DatabaseContent) -> Result<(), GeoSqliteError> {
    ...
    let written = write_content(&temporary_path, content);
    if written.is_err() {
