...
/// 座標列を、成分ごとに `scale` 倍して最近接整数へ量子化し、直前の点との差分（先頭の点は 0 との差分)を点ごとに第 1 成分・第 2 成分の順で zigzag varint として並べたバイト列へ符号化する。事前条件として、量子化後の各成分の値は `i64` の範囲に収まること。
#[must_use]
pub fn encode_coordinates(coordinates: &[(f64, f64)], scale: f64) -> Vec<u8> {
    ...
    for &(first, second) in coordinates {
        ...
        write_signed_varint(quantized.0 - previous.0, &mut bytes);
        write_signed_varint(quantized.1 - previous.1, &mut bytes);
        previous = quantized;
    }
